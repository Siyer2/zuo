...
git tag -a "v$build_version" -m "v$build_version" && git push origin "v$build_version"
link="https://github.com/nikitabobko/AeroSpace/releases/new?tag=v$build_version"
open "$link" || { echo "$link"; exit 1; }
sleep 1
open -R "./.release/Zuo-v$build_version.zip"
...
./script/build-brew-cask.sh \
    --cask-name zuo \
    --zip-uri "https://github.com/nikitabobko/AeroSpace/releases/download/v$build_version/Zuo-v$build_version.zip" \
    --build-version "$build_version"

eval "$cask_git_repo_path/pin.sh"
cp -r .release/zuo.rb "$cask_git_repo_path/Casks/zuo.rb"
