...
:url-search-for-config: https://github.com/search?q=path%3A*zuo.toml&type=code
...
Zuo provides bash, fish and zsh completions.
...
. Unpack zip
. Put unpacked `Zuo-v$VERSION/Zuo.app` to `/Applications`
. Put unpacked `Zuo-v$VERSION/bin/zuo` anywhere to `$PATH`
(The step is optional. It is only needed if you want to be able to interact with Zuo from CLI)

If you see this message

----
"Zuo.app" can't be opened because Apple cannot check it for malicious software.
----

**Option 1** to resolve the problem

```
xattr -d com.apple.quarantine /Applications/Zuo.app
```

**Option 2** to resolve the problem

. navigate in Finder to `/Applications/Zuo.app`
. Right mouse click
...
Zuo tries to find the custom config in two locations:

. `~/.zuo.toml`
. `+${XDG_CONFIG_HOME}/zuo/zuo.toml+`
...
Zuo uses TOML format for the config.
...
If no custom config is found, Zuo will load the default config.
...
[source,shell]
----
cp /Applications/Zuo.app/Contents/Resources/default-config.toml ~/.zuo.toml
----
...
The initial binding mode that Zuo starts out with is "main".
...
Commands are the thing you use to manipulate Zuo and query its state.

There are two ways on how you can use commands:

. Bind keys to run Zuo commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
* For `dvorak` and `colemak` users, Zuo offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `Zuo.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, Zuo falls back to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
Zuo stores all windows and containers in a tree.
Zuo tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in Zuo.
...
In total, Zuo provides 4 possible layouts:
...
By default, Zuo does two types of tree normalizations:
...
Since Spaces are so hard to deal with, Zuo reimplements Spaces and calls them "Workspaces".
...
When you quit the Zuo or when the Zuo detects that it's about to crash, Zuo will place all windows back to the visible area of the screen.

Zuo shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using Zuo workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if Zuo crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since Zuo needs a free space to hide windows in,
...
For some reason, mission control doesn't like that Zuo puts a lot of windows in the bottom right corner of the screen.
...
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs Zuo uses,
...
|👍 Public Apple API are more stable (which in turn affects Zuo stability)
...
|`if.during-zuo-startup`
a|
* If `true` then run the callback only during Zuo startup.
* If `false` then run callback only *NOT* during Zuo startup.
...
* `if.during-zuo-startup = true` is useful if you want to do the initial app arrangement only on startup.

* `if.during-zuo-startup = false` is useful if you want to relaunch Zuo, but the callback has side effects that you don’t want to run on every relaunch.
...
* You can use xref:commands.adoc#list-apps[`zuo list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger zuo_workspace_change FOCUSED=$ZUO_FOCUSED_WORKSPACE'
]
----

Besides the <<exec-env-vars,`exec.env-vars`>>, the process has access to the following environment variables:

* `ZUO_FOCUSED_WORKSPACE` - the workspace user switched to
* `ZUO_PREV_WORKSPACE` - the workspace user switched from

For a more elaborate example on how to integrate with Sketchybar see
xref:./goodies.adoc#show-zuo-workspaces-in-sketchybar[]
...
. Zuo takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. Zuo focuses the workspace
...
[NOTE]
====
The only difference between Zuo and i3 is switching to empty workspaces.
When you switch to an empty workspace, Zuo puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that Zuo model works better with <<observation,_the observation_>> listed above.
* Zuo model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
Zuo uses the API to gently ask windows whether they are dialogs, but Zuo also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, Zuo already does it.
Please see `isDialogHeuristic` function in Zuo sources.
...
If you can't make Zuo handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
