...
Do you have a cool automatization, Zuo integration, or workflow?
...
You can also use `after-startup-command` to start JankyBorders together with Zuo

[source,toml]
----
after-startup-command = [
    # JankyBorders has a built-in detection of already running process,
    # so it won't be run twice on Zuo restart
    'exec-and-forget borders active_color=0xffe1e3e4 inactive_color=0xff494d64 width=5.0'
]
----

[#raycast-extension]
== Zuo Raycast extension

There is a third party Raycast extension for Zuo. https://www.raycast.com/limonkufu/aerospace
...
[source,bash]
----
zuo workspace "$(zuo list-workspaces --monitor mouse --visible)" && zuo workspace next

zuo workspace "$(zuo list-workspaces --monitor mouse --visible)" && zuo workspace prev
----
...
Beware that you might need to specify full path to zuo executable https://community.folivora.ai/t/how-to-execute-terminal-command-to-switch-workspaces-with-aerospace/35914
* https://github.com/MediosZ/SwipeZuo
...
[#show-zuo-workspaces-in-sketchybar]
== Show Zuo workspaces in Sketchybar

You can integrate Zuo workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
Use these snippets as a starting point.

.~/.zuo.toml
[source,toml]
----
# Run Sketchybar together with Zuo
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger zuo_workspace_change FOCUSED_WORKSPACE=$ZUO_FOCUSED_WORKSPACE'
]
----
...
[source,bash]
----
sketchybar --add event zuo_workspace_change

for sid in $(zuo list-workspaces --all); do
    sketchybar --add item space.$sid left \
        --subscribe space.$sid zuo_workspace_change \
        --set space.$sid \
        ...
        label="$sid" \
        click_script="zuo workspace $sid" \
        script="$CONFIG_DIR/plugins/zuo.sh $sid"
done
----

.~/.config/sketchybar/plugins/zuo.sh
[source,bash]
----
#!/usr/bin/env bash

# make sure it's executable with:
# chmod +x ~/.config/sketchybar/plugins/zuo.sh
...
[#show-zuo-workspaces-in-simple-bar]
== Show Zuo workspaces in simple-bar

link:https://github.com/Jean-Tinland/simple-bar[simple-bar] can be used to display Zuo workspaces.

In order to sync simple-bar with Zuo, add these lines to your Zuo config file:
...
Opening *a new window* of a program that can support multiple windows (such as Safari or Terminal.app) can be accomplished with an AppleScript inlined in `zuo.toml` as follows:
