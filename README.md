# Zuo Beta [![Build](https://github.com/nikitabobko/AeroSpace/actions/workflows/build.yml/badge.svg?branch=main)](https://github.com/nikitabobko/AeroSpace/actions/workflows/build.yml)
...
Docs:
- [Zuo Guide](https://nikitabobko.github.io/Zuo/guide)
- [Zuo Commands](https://nikitabobko.github.io/Zuo/commands)
- [Zuo Goodies](https://nikitabobko.github.io/Zuo/goodies)

## Key features

- Tiling window manager based on a [tree paradigm](https://nikitabobko.github.io/Zuo/guide#tree)
- [i3](https://i3wm.org/) inspired
- Fast workspaces switching without animations and without the necessity to disable SIP
- Zuo employs its [own emulation of virtual workspaces](https://nikitabobko.github.io/Zuo/guide#emulation-of-virtual-workspaces) instead of relying on native macOS Spaces due to [their considerable limitations](https://nikitabobko.github.io/Zuo/guide#emulation-of-virtual-workspaces)
- Plain text configuration (dotfiles friendly). See: [default-config.toml](https://nikitabobko.github.io/Zuo/guide#default-config)
- CLI first (manpages and shell completion included)
- Doesn't require disabling SIP (System Integrity Protection)
- [Proper multi-monitor support](https://nikitabobko.github.io/Zuo/guide#multiple-monitors) (i3-like paradigm)
...
In multi-monitor setup please make sure that monitors [are properly arranged](https://nikitabobko.github.io/Zuo/guide#proper-monitor-arrangement).

Other installation options: https://nikitabobko.github.io/Zuo/guide#installation

> [!NOTE]
> By using Zuo, you acknowledge that it's not [notarized](https://developer.apple.com/documentation/security/notarizing_macos_software_before_distribution).
>
...
> "Apple cannot check Zuo for malicious software"
...
Public Beta. Zuo can be used as a daily driver, but expect breaking changes until 1.0 is reached.
...
**Values**
- Zuo is targeted at advanced users and developers
- Keyboard centric
- Breaking changes (configuration files, CLI, behavior) are avoided as much as possible, but it must not let the software stagnate.
  ...
  [Semver](https://semver.org/) major version is bumped in case of a breaking change (It's all guaranteed once Zuo reaches 1.0 version, until then breaking changes just happen)
- Zuo doesn't use GUI, unless necessarily
  - Zuo will never provide a GUI for configuration.
...
- "dark magic" (aka "private APIs", "code injections", etc.) must be avoided as much as possible
  - Right now, Zuo uses only a single private API to get window ID of accessibility object `_AXUIElementGetWindow`.
  ...
  - Zuo will never require you to disable SIP (System Integrity Protection).
  - The goal is to make Zuo easily maintainable, and resistant to macOS updates.

**Non Values**
- Play nicely with existing macOS features.
  If limitations are imposed then Zuo won't play nicely with existing macOS features
  (For example, Zuo doesn't acknowledge the existence of macOS Spaces, and it uses [emulation of its own workspaces](https://nikitabobko.github.io/Zuo/guide#emulation-of-virtual-workspaces))
- Ricing.
  Zuo provides only a very minimal support for ricing - gaps and a few callbacks for integrations with bars.
...
| ------------------------------------------------------------------------------ | ------------------ | ----------------- | ------------------ | ---------------- |
| Zuo binary runs on ...                                                   | +                  | +                 | +                  | +                |
| Zuo debug build from sources is supported on ...                         |                    | +                 | +                  | +                |
| Zuo release build from sources is supported on ... (Requires Xcode 26+)  |                    |                   | +                  | +                |

## Sponsorship

Zuo is developed and maintained in my free time.
