...
let usage =
    """
    USAGE: \(CommandLine.arguments.first ?? "zuo") [-h|--help] [-v|--version] <subcommand> [<args>...]
...
struct Main {
    static func main() async {
        ...
        if args.first == "--version" || args.first == "-v" {
            ...
            print(
                """
                zuo CLI client version: \(cliClientVersionAndHash)
                Zuo.app server version: \(serverVersionAndHash ?? "Unknown. The server is not running")
                """,
            )
            if serverVersionAndHash != nil && cliClientVersionAndHash != serverVersionAndHash {
                eprint(
                    """
                    Warning: Zuo client/server versions don't match. Possible fixes:
                      - Restart Zuo.app (server restart is required after each update)
                      - Reinstall and restart Zuo (corrupted installation)
                    """,
                )
            }
            exit(0)
        ...
        if let e = await connection.startBlocking().error {
            exit(1, err: "Can't connect to Zuo server. Is Zuo.app running?\n\(e.localizedDescription)")
        }
            ...
            && hasStdin()
        {
                ...
                parsedArgs is MoveNodeToWorkspaceCmdArgs && (parsedArgs as! MoveNodeToWorkspaceCmdArgs).explicitStdinFlag == nil
            {
                exit(
                    1,
                    err: """
                        ERROR: Implicit stdin is detected (stdin is not TTY). Implicit stdin was forbidden in Zuo v0.20.0.
                        1. Please supply '--stdin' flag to make stdin explicit and preserve old Zuo behavior
        ...
        let windowId = ProcessInfo.processInfo.environment[ZUO_WINDOW_ID].flatMap(UInt32.init)
        let workspace = ProcessInfo.processInfo.environment[ZUO_WORKSPACE]
        ...
        if ans.exitCode != 0 && ans.serverVersionAndHash != cliClientVersionAndHash {
            eprint(
                """
                Warning: Zuo client/server versions don't match
                  - zuo CLI client version: \(cliClientVersionAndHash)
                  - Zuo.app server version: \(ans.serverVersionAndHash)
                  Possible fixes:
                  - Restart Zuo.app (server restart is required after each update)
                  - Reinstall and restart Zuo (corrupted installation)
                """,
