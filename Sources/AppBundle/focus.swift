...
/// But, in general, commands must firstly check --window-id, --workspace, ZUO_WINDOW_ID env and
/// ZUO_WORKSPACE env before accessing the global focus.
...
@MainActor private func onWorkspaceChanged(_ oldWorkspace: String, _ newWorkspace: String) {
    ...
    if let exec = config.execOnWorkspaceChange.first {
        ...
        environment["ZUO_FOCUSED_WORKSPACE"] = newWorkspace
        environment["ZUO_PREV_WORKSPACE"] = oldWorkspace
        environment[ZUO_WORKSPACE] = newWorkspace
