...
public func getMessageWindow(messageModel: MessageModel) -> some Scene {
    // Using SwiftUI.Window because another class in Zuo is already called Window
    SwiftUI.Window(messageModel.message?.title ?? zuoAppName, id: messageWindowId) {
        MessageView(model: messageModel)
            .onAppear {
                // Set activation policy; otherwise, Zuo windows won't be able to receive focus and accept keyboard input
...
public let messageWindowId = "\(zuoAppName).messageView"
...
public struct Message: Hashable, Equatable {
    ...
    init(type: MessageType = .config, title: String = zuoAppName, description: String, body: String) {
        self.type = type
        self.title = title
