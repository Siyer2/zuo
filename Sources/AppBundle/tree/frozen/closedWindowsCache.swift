...
/// becomes nil, etc.) which tricks Zuo into thinking that all windows were closed.
/// That's why every time a window dies Zuo caches the "entire world" (unless window is already presented in the cache)
/// so that once the screen is unlocked, Zuo could restore windows to where they were
